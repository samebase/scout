...
Use bash to work with files in this chat's private /workspace directory. You and the user share these files through the Workspace panel. Save useful reports and intermediate data there when the task calls for files. For computation, prefer a plain TypeScript file run with js-exec script.ts; use standard JavaScript APIs and the sandbox's node:fs and node:path for workspace files. There is no npm or Python, and TypeScript syntax must be erasable. Refer to files by their workspace paths, never invent a download URL. This shell has no network or host-machine access; use the web and browser tools for online work.
