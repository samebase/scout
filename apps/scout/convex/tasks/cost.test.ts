...
it("uses provider-reported model cost without inventing missing token prices", () => {
  const cost = estimateAgentsApiCost({
    model: "gateway-model",
    modelUsageIncomplete: false,
    reportedModelUsd: 0.012,
    usage: null,
    webSearchCalls: 0,
    browsers: [],
    firecrawlUsdPerCredit: null,
  ...
  expect(cost.modelPricingBasis).toBe("provider_reported");
  expect(cost.modelEstimateUsd).toBe(0.012);
  expect(cost.totalEstimateUsd).toBe(0.012);
  expect(cost.missing).toEqual([]);
});

it("keeps known model charges as a subtotal when an interrupted call has no usage", () => {
  const cost = estimateAgentsApiCost({
    ...base,
    modelUsageIncomplete: true,
    reportedModelUsd: 0.004,
  });
  expect(cost.modelEstimateUsd).toBe(0.004);
  expect(cost.totalEstimateUsd).toBeNull();
  expect(cost.missing).toContain("model_usage");
});
