...
export async function saveScreenshot(
  ...
  const captureId = await ctx.runMutation(internal.tasks.screenshotRecords.prepare, {
  ...
  try {
    ...
    await ctx.runMutation(internal.tasks.screenshotRecords.finish, {
    ...
    await ctx.runMutation(internal.tasks.screenshotRecords.fail, {
      screenshotId: captureId,
      message,
    });
...
export const imageUrl = publicAction({
  ...
  handler: async (ctx, args) => {
    const key = await ctx.runQuery(internal.tasks.screenshotRecords.imageKey, args);
    if (!key) return null;
    const expiresIn = 900;
