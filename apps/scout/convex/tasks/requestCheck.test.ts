...
async function setup() {
  ...
  const inspect = () => admin.query(api.tasks.sessions.get, { sessionId });
  const inspectCheck = () => admin.query(api.tasks.requestChecks.inspect, { sessionId, checkId });
  const check = () => backend.action(internal.tasks.requestCheck.run, { checkId });
  const publicFeed = () =>
...
it("saves the real call and generated title, then makes an approved review public", async () => {
  ...
  await expect(
    t.member.query(api.tasks.sessions.get, { sessionId: t.sessionId }),
  ).rejects.toThrow();
...
it("charges measured usage even when the task is stopped during the check", async () => {
  ...
  vi.stubGlobal(
    "fetch",
    vi.fn<typeof fetch>(async () => {
      await t.member.mutation(api.tasks.sessions.stop, { sessionId: t.sessionId });
