...
async function setup(prompt = "Try https://example.com and tell me whether it works.") {
  ...
  function task(sessionId: Id<"agentsApiSessions">, checkId: Id<"agentsApiRequestChecks">) {
    return {
      sessionId,
      checkId,
      run: () => backend.action(internal.agentsApi.siteResearch.run, { sessionId, prompt }),
      advance: () => backend.action(internal.agentsApi.siteResearch.advance, { sessionId }),
      inspect: () => admin.query(api.agentsApi.siteResearchRecords.inspect, { sessionId }),
    };
  }
  ...
  async function sharedJob() {
    ...
    const research = await backend.query(internal.agentsApi.siteResearchRecords.job, {
  ...
  return {
    ...
    process: (researchId: Id<"agentsApiSiteResearch">) =>
      backend.action(internal.agentsApi.siteResearch.process, { researchId }),
    refresh: () => admin.action(api.agentsApi.siteResearch.refresh, { site: "example.com" }),
  };
}
...
it("lets concurrent tasks share one paid job and attributes its credits only once", async () => {
  ...
  expect(scheduled.filter((job) => job.name === "agentsApi/siteResearch:process")).toHaveLength(1);
...
it.each(["saving request", "submitting job"])(
  ...
  async (stage) => {
    ...
    await second.run();
    const shared = await t.sharedJob();
    const stop = () => t.admin.mutation(api.agentsApi.sessions.stop, { sessionId: t.sessionId });
...
it("serializes concurrent refreshes and ignores a stale expected research ID", async () => {
  ...
  await t.process(firstId);
  expect(
    await t.backend.mutation(internal.agentsApi.siteResearchRecords.refresh, {
      site: "example.com",
      userId: t.userId,
...
it("requires admin access for refresh and inspect without creating unauthorized work", async () => {
  ...
  for (const viewer of [t.backend, t.member]) {
    await expect(
      viewer.action(api.agentsApi.siteResearch.refresh, { site: "example.com" }),
    ).rejects.toThrow("Not authorized");
    await expect(
      viewer.query(api.agentsApi.siteResearchRecords.inspect, { sessionId: t.sessionId }),
    ).rejects.toThrow("Not authorized");
  }
  expect(await t.records()).toEqual(before);
  expect((await t.site())?.researchId).toBe(shared._id);
...
it.each([
  ...
  await t.run();
  const before = await t.records();
  await expect(t.admin.action(api.agentsApi.siteResearch.refresh, { site })).rejects.toThrow();
  expect(await t.records()).toEqual(before);
  expect(startAgent).not.toHaveBeenCalled();
});
