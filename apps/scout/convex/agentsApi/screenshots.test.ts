...
async function setup(visibility: Doc<"scoutChats">["visibility"]) {
  ...
  const reserve = async (toolCallId: string, providerSessionId: string) => {
    await backend.mutation(internal.agentsApi.browsers.prepareOperation, {
      providerSessionId,
      toolCallId,
      action: { kind: "open", url: image.metadata.url },
    });
    return await backend.mutation(internal.agentsApi.screenshotRecords.prepare, {
      sessionId: task.sessionId,
      providerSessionId,
      toolCallId,
      note,
  ...
  const finish = (screenshotId: Id<"agentsApiScreenshots">) =>
    backend.mutation(internal.agentsApi.screenshotRecords.finish, {
      screenshotId,
      key: `private-images/${screenshotId}.png`,
      metadata: image.metadata,
    });
  const close = (providerSessionId: string) =>
    backend.mutation(internal.agentsApi.browsers.close, {
      providerSessionId,
...
it("lets a private task's owner and admin read metadata and sign its image", async () => {
  ...
  for (const viewer of [t.owner, t.admin]) {
    expect(await viewer.query(api.agentsApi.walkthrough.get, { sessionId: t.sessionId })).toEqual({
      walkthrough: null,
      captures: [
        {
          id: captureId,
    ...
    expect(
      await viewer.action(api.agentsApi.screenshots.imageUrl, { screenshotId: captureId }),
    ).toEqual({
      url: signedUrl,
...
it("denies private metadata and image signing by capture ID to other users and anonymous viewers", async () => {
  ...
  for (const viewer of [t.other, t.backend]) {
    expect(
      await viewer.query(api.agentsApi.walkthrough.get, { sessionId: t.sessionId }),
    ).toBeNull();
    expect(
      await viewer.action(api.agentsApi.screenshots.imageUrl, { screenshotId: captureId }),
    ).toBeNull();
...
it("does not sign pending, failed, or deleted captures even for the owner", async () => {
  ...
  await t.backend.mutation(internal.agentsApi.screenshotRecords.fail, {
    screenshotId: failed,
    message: "Capture failed",
  });
  await t.backend.run((ctx) => ctx.db.delete(deleted));
  for (const screenshotId of [pending, failed, deleted]) {
    expect(await t.owner.action(api.agentsApi.screenshots.imageUrl, { screenshotId })).toBeNull();
  }
  expect(getUrl).not.toHaveBeenCalled();
});
...
it("orders captures by browser and operation sequence despite reversed reservations and upload completion", async () => {
  ...
  for (const toolCallId of ["first", "second"]) {
    await t.backend.mutation(internal.agentsApi.browsers.prepareOperation, {
  ...
  const captures = await t.backend.query(internal.agentsApi.walkthrough.listForAgent, {
  ...
  expect(await t.owner.query(api.agentsApi.walkthrough.get, { sessionId: t.sessionId })).toEqual({
    walkthrough: null,
    captures,
  });
...
it("rejects duplicate reservations without allocating another capture or replacing its note", async () => {
  ...
  await expect(
    t.backend.mutation(internal.agentsApi.screenshotRecords.prepare, args),
  ...
  expect(
    await t.backend.query(internal.agentsApi.walkthrough.listForAgent, { sessionId: t.sessionId }),
...
it("retains report references and renewable image access after browser closure and task completion", async () => {
  ...
  const view = await t.backend.query(api.agentsApi.walkthrough.get, { sessionId: t.sessionId });
  expect(view?.walkthrough).toEqual(report);
  expect(view?.captures.map(({ id }) => id)).toEqual([first, second]);
  expect(
    await t.backend.action(api.agentsApi.screenshots.imageUrl, { screenshotId: second }),
  ).toEqual({ url: signedUrl, expiresAtMs: Date.now() + 900_000 });
