...
async function setup() {
  ...
  const session = () =>
    backend.query(internal.agentsApi.sessions.cleanupResources, { sessionId: ids.sessionId });
...
it("queues a check for this handoff and denies duplicate Resume while checking", async () => {
  ...
  expect(
    (await t.admin.query(api.agentsApi.sessions.get, { sessionId: t.sessionId })).checks,
  ).toEqual([
...
it("allows only the approved owner to resume and only Lab viewers to inspect check evidence", async () => {
  ...
  for (const viewer of [t.backend, t.other, t.admin]) {
    await expect(
      viewer.mutation(api.agentsApi.sessions.resume, {
        sessionId: t.sessionId,
  ...
  for (const viewer of [t.backend, t.other, t.owner]) {
    await expect(
      viewer.query(api.agentsApi.requestChecks.inspect, {
        sessionId: t.sessionId,
        checkId,
      }),
...
it("does not inspect another session's check even for an administrator", async () => {
  ...
  const checkId = await t.resume();
  await expect(
    t.admin.query(api.agentsApi.requestChecks.inspect, {
      sessionId: otherSessionId,
      checkId,
    }),
  ).rejects.toThrow("Check not found in this session");
  await expect(
    t.backend.mutation(internal.agentsApi.requestChecks.releaseHandoff, {
      sessionId: otherSessionId,
      checkId,
    }),
...
it("keeps history through rejection, an approved retry, and a later handoff on the same provider", async () => {
  ...
  const session = await t.admin.query(api.agentsApi.sessions.get, { sessionId: t.sessionId });
  ...
  const listed = await t.admin.query(api.agentsApi.sessions.list, { paginationOpts });
...
it.each(["capture", "LLM"])(
  ...
  async (phase) => {
    ...
    expect(
      await t.backend.mutation(internal.agentsApi.requestChecks.releaseHandoff, {
        sessionId: t.sessionId,
        checkId,
      }),
    ).toBeNull();
...
it("releases the Scout only after the provider confirms browser cleanup", async () => {
  ...
  const cleanup = t.backend.action(internal.agentsApi.runtime.cleanup, { sessionId: t.sessionId });
  ...
  try {
    ...
    await expect(
      t.owner.mutation(api.agentsApi.sessions.send, {
        sessionId: t.sessionId,
