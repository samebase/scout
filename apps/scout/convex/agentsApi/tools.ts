...
export async function runtimeTools(
  ...
  const beforeDispatch = async () => {
    const current = await ctx.runQuery(internal.agentsApi.sessions.runtime, { sessionId });
  ...
  async function saveBrowser() {
    await ctx.runMutation(internal.agentsApi.sessions.update, { sessionId, browser: handle });
  }
  ...
  const browser = createBrowserHarness({
    ...
    onSessionCreated: async (created) => {
      ...
      await ctx.runMutation(internal.agentsApi.browsers.open, { sessionId, browser: handle });
    ...
    onOperationPrepared: async (operation) => {
      await beforeDispatch();
      if (!handle) throw new Error("Browser was not registered");
      return await ctx.runMutation(internal.agentsApi.browsers.prepareOperation, {
        providerSessionId: handle.providerSessionId,
        ...operation,
      });
    },
    onOperationSettled: async ({ outcome, selectedTabId, toolCallId, clickCapture }) => {
      if (!handle) throw new Error("Browser was not registered");
      await ctx.runMutation(internal.agentsApi.browsers.settleOperation, {
        providerSessionId: handle.providerSessionId,
        outcome,
        toolCallId,
        clickCapture,
