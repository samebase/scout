...
Store `role`, `status`, and `isApproved` directly on `users`, as Samebase does for approval.
The initial separate `accountAccess` proposal was unnecessary for one global assignment per user
and is superseded by this simpler layout. Keep audit events in `accountAccessAudit`.

New users receive member, active, and unapproved defaults in Convex Auth's
`afterUserCreatedOrUpdated` callback only when `existingUserId` is null. Existing users retain
all assignments through verification, login, and password recovery. Missing approval means
pending access. No separate account-row creation or initialization mutation is needed.

The viewer resolver reads the authenticated user once. Product access requires verification,
approval, and active status. The Members page updates the same user record. An index on role
and approval supports transactional last-admin checks, including users whose missing status
means active. Future resource ownership remains a separate product decision.
