...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Signup and approval           | `src/components/auth-panel.tsx`, `convex/auth.ts`, `convex/authEmails.ts`, `convex/accounts.ts`                                                                    | Email/password registration, verification and reset codes, manual approval, and saved task-engine/Scout preferences. Signup requires explicit terms acceptance. The acceptance timestamp is stored on the user; an age declaration is not stored. |
...
Signup requires an unchecked terms checkbox. The server rejects signup without acceptance
and stores a server timestamp in `users.termsAcceptedAt`. Existing accounts must accept
before using account features; policy pages, sign-out, and account closure remain available.
Repeat acceptance does not replace the original timestamp. The field is removed with the
profile during account closure. The privacy policy remains an informational notice, separate
from consent.
