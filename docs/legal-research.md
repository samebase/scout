...
- [Terms and conditions](../apps/scout/src/content/terms/2026-09-19.md), rendered at `/terms`.
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Signup and approval           | `src/components/auth-panel.tsx`, `convex/auth.ts`, `convex/authEmails.ts`, `convex/accounts.ts`                                                                    | Email/password registration, verification and reset codes, manual approval, and saved task-engine/Scout preferences. Signup requires explicit terms acceptance. Versioned acceptance records are stored; an age declaration is not stored. |
...
Signup requires an unchecked terms checkbox. The server rejects missing or stale versions and
records the account ID, terms version, and server timestamp in `termsAcceptances`. The current
version on the user record controls access through the shared permission layer. Existing
accounts must accept before using account features; policy pages, sign-out, and account
closure remain available. Repeat acceptance does not replace the original timestamp.

The published terms are preserved in `src/content/terms/2026-09-19.md`. For a future revision,
add a new dated document, update the `/terms` import and `shared/terms.ts` version, and keep
previous documents and acceptance records. Review notice requirements before changing the
required version. The privacy policy remains an informational notice, separate from consent.
